...
import { getCrmLeadDetail, getCrmOwners } from "../../_actions/leads";
import LeadDetailClient from "./LeadDetailClient";

export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const [result, ownersResult] = await Promise.all([getCrmLeadDetail(id), getCrmOwners()]);

  if (!result.data.lead) notFound();

  const { lead, owner, tasks, timeline } = result.data;
