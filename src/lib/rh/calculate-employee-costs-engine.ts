/**
 * Motor financeiro / payroll PT (indicativo) — mesma lógica que a Edge Function Deno.
 * Executado no Node (rota API Next.js) para funcionar sem deploy em Supabase Functions.
 */
...
export type CalcPayload = Record<string, JsonPrimitive | unknown>

function toDecimal(value: unknown, field: string, warnings: string[]): Decimal {
...
function calcTsu(
  ...
  const incidence = includeMeal ? base.plus(bonuses).plus(meal) : base.plus(bonuses)
...
function calcIndemnitySafeguard(
  ...
  const indemnityValue = Decimal.min(indemnityByDays, valueCapByMonths)
...
/**
 * Executa o cálculo. Corpo alinhado com a resposta JSON da Edge Function.
 */
export function runCalculateEmployeeCosts(body: CalcInput): { status: number; payload: CalcPayload } {
  ...
  if (!contractStart) {
    return {
      status: 400,
      payload: { ok: false, error: 'contract_start_date obrigatória e válida (ISO 8601).', warnings },
    }
  }
  ...
  if (body.reference_date && !refRaw) {
    return {
      status: 400,
      payload: { ok: false, error: 'reference_date inválida.', warnings },
    }
  }
  ...
  const subsidies = calcSubsidiesProportional(referenceMonthly, contractStart, reference, warnings)
  const termination = calcIndemnitySafeguard(referenceMonthly, contractStart, reference, warnings)
  ...
  const monthlyEmployerCostEstimate = roundMoney(monthlyEmployerCash.plus(tsu.employerAmount))

  if (baseSalary.isZero() && bonuses.isZero()) {
    warnings.push('Salário base e prémios são zero — resultados são meramente ilustrativos.')
  }
